<?php
/**
 * ชื่อไฟล์: LoginService.php
 * คำอธิบาย: Service สำหรับจัดการขั้นตอนการเข้าสู่ระบบ/ออกจากระบบของพนักงาน และการดึงผู้ใช้ที่ล็อกอินอยู่ในปัจจุบัน
 * ผู้เขียน: Yothin S.
 * วันที่แก้ไข: 20/02/2026
 */

namespace App\Service\LoginServices;
...
class LoginService
{
    ...
    public function loginSession(Employee $employee, Request $request): array
    {
        try {
            ...
            Log::error('Login system error', [
                'error' => $e->getMessage(),
                'email' => $employee->getAttribute('emp_email'),
            ]);
