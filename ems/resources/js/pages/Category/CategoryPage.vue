    ...
    methods: {
        ...
        async loadCategories() {
            try {
                ...
                // 2) สร้าง Set ของ categoryId ที่ถูกใช้ใน event
                // ใช้ field ตามหน้า Event: evn_cat_id หรือ evn_category_id
                const usedCatIds = new Set(
                    events
                        .map((e) =>
                            String(
                                e.evn_cat_id ?? e.evn_category_id ?? ""
                            ).trim()
                        )
                        .filter(Boolean)
                );

                // 3) map categories แล้วใส่ is_used_in_event ให้ row
        ...
        async createCategory({ name }) {
            try {
                ...
                const msg =
                    err?.response?.data?.message || "Cannot create category.";
                this.alert = {
                    open: true,
                    type: "error",
                    title: "Failed",
        ...
        isDupForEdit(name, currentId) {
            ...
            return this.rows.some(
                (r) =>
                    r.id !== currentId && (r.cat_name || "").toLowerCase() === n
            );
        },
