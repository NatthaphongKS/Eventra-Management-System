    ...
    <div class="font-[Poppins]" @pointerdown.capture="onRootPointer">
        ...
        <div class="grid grid-cols-12 gap-8">
            <div class="col-span-12 lg:col-span-8">
                <div class="grid md:grid-cols-[1fr_240px] gap-6 items-start">
                    <div>
                        ...
                        <InputPill v-model="eventTitle" placeholder="Enter event title"
                ...
                <div class="mt-6">
                    ...
                    <textarea v-model.trim="eventDescription" placeholder="Enter description..."
                        class="border border-neutral-200 w-full h-[160px] rounded-2xl p-5 font-medium text-neutral-800 focus:outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-300 transition resize-none"></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <div class="flex h-[52px] w-full items-center gap-1 rounded-2xl border border-neutral-200 px-3 shadow-sm bg-white focus-within:ring-2 focus-within:ring-rose-300 focus-within:border-rose-400 transition">
                            <input type="time" v-model="eventTimeStart" step="300"
                                ...
                                @click="$event.target.showPicker()" @keydown.prevent />
                ...
                <div class="mt-6">
                    ...
                    <InputPill v-model="eventLocation" placeholder="Enter location"
                        class="w-full h-[52px] font-medium text-[16px] text-neutral-800 border border-neutral-200 rounded-[20px] px-5 focus:outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-300 transition" />
                </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-4">
                ...
                <div class="group relative flex flex-col min-h-[300px] rounded-[24px] border-2 border-dashed border-rose-200 bg-rose-50/50 p-5 transition-all hover:border-rose-400 hover:bg-rose-50"
        ...
        <div class="mt-8 flex justify-end gap-4 border-t border-dashed border-neutral-200 pt-6">
            <button type="button" @click="onCancel" :disabled="saving"
                class="h-[48px] min-w-[120px] rounded-2xl border border-neutral-300 px-6 font-semibold text-neutral-600 hover:bg-neutral-50 hover:text-neutral-800 transition disabled:opacity-50">
                Cancel
            </button>

            <button type="button" @click="saveEvent" :disabled="saving"
                class="inline-flex h-[48px] min-w-[140px] items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-6 font-semibold text-white shadow-md hover:bg-emerald-700 transition disabled:opacity-60 disabled:cursor-not-allowed">
                <Icon icon="ic:baseline-plus" class="w-5 h-5 text-white" />
                <span>{{ saving ? "Saving..." : "Create Event" }}</span>
            </button>
        </div>
...
export default {
    components: { InputPill, Icon, DataTable, ModalAlert, SearchBar, EmployeeDropdown },
    data() {
        return {
            // Form Data
            eventTitle: "",
    ...
    computed: {
        columns() {
            return [
                { key: "emp_id", label: "ID", sortable: false, class: "min-w-[100px]" },
        ...
        filteredEmployees() {
            ...
            if (q) {
                list = list.filter((e) =>
                    [String(e.emp_id), e.emp_firstname, e.emp_lastname, e.nickname]
            ...
            if (this.selectedCompanyIds?.length) {
                ...
                list = list.filter((r) => {
                     ...
                     const idStr = String(r.companyId || r.companyAbbr || "").trim();
    ...
    methods: {
        ...
        addFiles(files) {
             const MAX_MB = 50;
            const ALLOW = [
                "application/pdf", "text/plain", "application/msword", "image/jpeg", "image/png",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            ];
            const errs = [];
            files.forEach((f) => {
                if (f.size > MAX_MB * 1024 * 1024) errs.push(`${f.name}: File too large (> ${MAX_MB}MB)`);
                else if (!ALLOW.includes(f.type)) errs.push(`${f.name}: Invalid file type`);
        ...
        async saveEvent() {
             if (!this.eventTitle || !this.eventCategoryId || !this.eventDate || !this.eventTimeStart || !this.eventTimeEnd || !this.eventLocation) {
                this.openAlert({ type: "warning", title: "Missing Info", message: "Please fill in all required fields (*).", });
                return;
            }
            this.openAlert({
                type: "confirm",
        ...
        openAlert(cfg) {
            this.alert = {
                ...
                onConfirm: () => { this.alert.open = false; },
                ...cfg,
            };
        },
