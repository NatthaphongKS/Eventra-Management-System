            ...
            <div class="col-span-12 lg:col-span-8">
                <div class="grid md:grid-cols-[1fr_240px] gap-6 items-start">
                    <div>
                        ...
                        <InputPill
                            ...
                            :class="['w-full h-[52px] font-medium text-[16px] text-neutral-800 border rounded-[20px] px-5 focus:outline-none transition',
                                    errors.eventTitle ? 'border-red-500 ring-1 ring-red-200' : 'border-neutral-200 focus:border-rose-400 focus:ring-2 focus:ring-rose-300']"
                        />
                        <p v-if="errors.eventTitle" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required field</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select
                                ...
                                :class="['appearance-none border rounded-[20px] px-[20px] w-full h-[52px] font-medium text-neutral-800 focus:outline-none transition bg-white cursor-pointer',
                                        errors.eventCategoryId ? 'border-red-500 ring-1 ring-red-200' : 'border-neutral-200 focus:border-rose-400 focus:ring-2 focus:ring-rose-300']"
                            >
                            ...
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-neutral-500">
                                <Icon icon="mdi:chevron-down" class="h-6 w-6 text-neutral-400" />
                            </div>
                        </div>
                        <p v-if="errors.eventCategoryId" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required Select</p>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea
                        ...
                        :class="['w-full h-[160px] rounded-2xl p-5 font-medium text-neutral-800 focus:outline-none transition resize-none border',
                                errors.eventDescription ? 'border-red-500 ring-1 ring-red-200' : 'border-neutral-200 focus:border-rose-400 focus:ring-2 focus:ring-rose-300']"
                    ></textarea>
                    <p v-if="errors.eventDescription" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required field</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    <div>
                        ...
                        <div class="relative group">
                            <div
                                :class="['absolute inset-0 flex items-center px-[20px] font-medium rounded-2xl pointer-events-none z-10 bg-white border transition',
                                        eventDate ? 'text-neutral-800' : 'text-neutral-400',
                                        errors.eventDate ? 'border-red-500' : 'border-neutral-200 group-focus-within:border-rose-400']"
                            >
                        ...
                        <p v-if="errors.eventDate" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required date</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div
                            :class="['flex h-[52px] w-full items-center gap-1 rounded-2xl border px-3 shadow-sm bg-white transition',
                                    errors.eventTime ? 'border-red-500 ring-1 ring-red-200' : 'border-neutral-200 focus-within:ring-2 focus-within:ring-rose-300 focus-within:border-rose-400']"
                        >
                            <input
                                type="time"
                                ...
                                @keydown.prevent
                            />
                            <span class="text-[16px] font-bold text-neutral-400">:</span>
                            <input
                                type="time"
                                ...
                                @keydown.prevent
                            />
                            <Icon icon="mdi:clock-outline" class="w-6 h-6 text-neutral-400 ml-1 pointer-events-none" />
                        </div>
                        <p v-if="errors.eventTime" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required time</p>
                    </div>

                    <div>
                        <label class="block text-neutral-800 font-semibold text-[15px] mb-2">Duration</label>
                        <div class="flex h-[52px] w-full items-center gap-3 rounded-2xl border border-neutral-200 px-4 shadow-sm bg-[#F9FAFB]">
                            <input class="w-full h-full bg-transparent font-medium text-neutral-600 outline-none" disabled v-model="eventDurationDisplay" />
                ...
                <div class="mt-6">
                    ...
                    <InputPill
                        ...
                        :class="['w-full h-[52px] font-medium text-[16px] text-neutral-800 border rounded-[20px] px-5 focus:outline-none transition',
                                errors.eventLocation ? 'border-red-500 ring-1 ring-red-200' : 'border-neutral-200 focus:border-rose-400 focus:ring-2 focus:ring-rose-300']"
                    />
                    <p v-if="errors.eventLocation" class="text-red-500 text-xs mt-1.5 ml-2 font-medium italic">Required date</p>
                </div>
            </div>

            <div class="col-span-12 lg:col-span-4">
                <label class="block text-neutral-800 font-semibold text-[15px] mb-2">Upload attachments</label>
                <p class="text-xs text-neutral-500 mb-3">Drag and drop document to your support task</p>
                <div
                    ...
                    <div v-if="filesNew.length > 0" class="flex flex-col gap-2 mb-4">
                        <div v-for="(item, index) in filesNew" :key="index" class="w-full flex items-center justify-between rounded-2xl bg-white border border-rose-100 px-3 py-2.5 shadow-sm">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="grid h-10 w-10 place-items-center rounded-xl bg-rose-100 text-rose-600">
                                    <Icon icon="basil:file-solid" class="h-6 w-6" />
                                </div>
                                <div class="truncate">
                            ...
                            <button type="button" class="grid h-7 w-7 place-items-center rounded-full text-neutral-400 hover:text-red-600 hover:bg-rose-50 transition" @click="removeFile(index)">
                                <Icon icon="mdi:close" class="h-4 w-4" />
                            </button>
                        </div>
                    </div>
                    <div v-else class="flex flex-1 flex-col items-center justify-center text-center">
                        <div class="mb-3 grid h-16 w-16 place-items-center rounded-full bg-rose-100 text-rose-500">
                            <Icon icon="entypo:upload-to-cloud" class="h-8 w-8" />
                        </div>
                        <p class="text-sm font-medium text-neutral-800">Choose a file or drag &amp; drop it here</p>
                        <p class="mt-1 text-xs text-neutral-500">pdf, txt, docx, jpeg, xlsx</p>
                    </div>
                    <div class="mt-auto flex justify-center pt-4">
                        <button type="button" class="inline-flex items-center rounded-xl border border-rose-200 bg-white px-4 py-2 text-sm font-semibold text-rose-700 shadow-sm hover:bg-rose-50 hover:border-rose-300 transition" @click="pickFiles">
                            Browse files
                        </button>
                    </div>
        ...
        <div class="mt-10">
            ...
            <div class="mt-6">
                ...
                >
                    ...
                    <template #empty>
                        <div class="py-8 text-center text-neutral-400">ไม่พบข้อมูลพนักงาน</div>
                    </template>
                </DataTable>
            </div>
        </div>

        <div class="mt-10 w-full flex justify-between items-center border-t border-neutral-100 pt-6">
            <div class="flex-none">
...
export default {
    components: {
        InputPill, Icon, DataTable, ModalAlert, SearchBar, EmployeeDropdown, CancelButton,
    },
    data() {
        return {
            eventTitle: "",
            ...
            saving: false,
            alert: {
                open: false, type: "confirm", title: "", message: "", showCancel: false, onConfirm: null,
            },
        };
    },
    computed: {
        ...
        filteredEmployees() {
            ...
            if (q) {
                list = list.filter((e) => [String(e.emp_id), e.emp_firstname, e.emp_lastname, e.nickname].some((f) => f?.toLowerCase().includes(q)));
            }
            if (this.selectedCompanyIds?.length) {
                const needles = this.selectedCompanyIds.map((x) => String(x).trim()).filter(Boolean);
                list = list.filter((r) => needles.some((n) => String(r.companyId || r.companyAbbr || "").includes(n)));
            }
    ...
    watch: {
        ...
        eventTitle() { if(this.eventTitle) this.errors.eventTitle = false; },
        eventCategoryId() { if(this.eventCategoryId) this.errors.eventCategoryId = false; },
        eventDescription() { if(this.eventDescription) this.errors.eventDescription = false; },
        eventDate() { if(this.eventDate) this.errors.eventDate = false; },
        eventLocation() { if(this.eventLocation) this.errors.eventLocation = false; },
        eventTimeStart() { if(this.eventTimeStart && this.eventTimeEnd) this.errors.eventTime = false; },
        eventTimeEnd() { if(this.eventTimeStart && this.eventTimeEnd) this.errors.eventTime = false; },
    },
    mounted() {
        this.fetchInfo();
    },
    methods: {
        async fetchInfo() {
            try {
                ...
                this.employees = (data.employees || []).map((e) => {
                    ...
                    return {
                        id: e.id, emp_id: e.emp_id || "", emp_firstname: e.emp_firstname || "",
                        emp_lastname: e.emp_lastname || "", nickname: e.emp_nickname || "",
                        department: e.department_name || "", team: e.team_name || "",
                        position: e.position_name || "", companyAbbr: rawPrefix.toUpperCase(),
                        companyId: e.company_id || rawPrefix.toUpperCase() || "",
                    };
                });
        ...
        calDuration() {
            if (!this.eventTimeStart || !this.eventTimeEnd) {
                this.eventDurationDisplay = ""; this.eventDurationMinutes = 0; return;
            }
        ...
        addFiles(files) {
            const MAX_MB = 50;
            const ALLOW = ["application/pdf", "text/plain", "application/msword", "image/jpeg", "image/png", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
            files.forEach((f) => {
                if (f.size <= MAX_MB * 1024 * 1024 && ALLOW.includes(f.type)) this.filesNew.push(f);
            });
        },
        ...
        onCancel() {
            if (this.filesNew.length || this.eventTitle) { if (!confirm("Discard unsaved changes?")) return; }
            this.$router.back();
        },
            ...
            this.openAlert({
                ...
                onConfirm: async () => {
                    ...
                    try {
                        ...
                        this.openAlert({
                            type: "success", title: "CREATE SUCCESS!", message: "New event has been created.",
                            onConfirm: () => this.$router.push("/event"),
                        });
                    } catch (err) {
                        const msg = err.response?.data?.message || "Failed to create event.";
                        this.openAlert({ type: "error", title: "ERROR", message: msg });
                    } finally { this.saving = false; }
                },
            });
        },
        openAlert(cfg) {
            this.alert = { ...this.alert, open: true, showCancel: false,
                onConfirm: () => { this.alert.open = false; }, ...cfg
            };
        },
        onRootPointer(e) {},
    },
...
.time-input::-webkit-calendar-picker-indicator { opacity: 0; }
