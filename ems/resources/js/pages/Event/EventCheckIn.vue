...
<template>
  ...
  <div class="m-6 flex flex-col gap-3">
    ...
    <div class="mb-4 flex flex-wrap items-center gap-2">
      <button class="status-pill" :class="statusFilter === 'accepted' ? 'ring-2 ring-emerald-300' : ''"
        @click="setStatus('accepted')">
        <span class="inline-flex h-2 w-2 rounded-full bg-emerald-500"></span>
        <span class="ml-2">{{ counts.accepted }}/{{ totals.accepted }} Accepted</span>
      </button>

      <button class="status-pill" :class="statusFilter === 'declined' ? 'ring-2 ring-rose-300' : ''"
        @click="setStatus('declined')">
        <span class="inline-flex h-2 w-2 rounded-full bg-rose-500"></span>
        <span class="ml-2">{{ counts.declined }}/{{ totals.declined }} Declined</span>
      </button>

      <button class="status-pill" :class="statusFilter === 'pending' ? 'ring-2 ring-amber-300' : ''"
        @click="setStatus('pending')">
        <span class="inline-flex h-2 w-2 rounded-full bg-amber-500"></span>
        <span class="ml-2">{{ counts.pending }}/{{ totals.pending }} Pending</span>
      </button>

      <button class="status-pill" :class="statusFilter === 'notinvited' ? 'ring-2 ring-slate-300' : ''"
        @click="setStatus('notinvited')">
        <span class="inline-flex h-2 w-2 rounded-full bg-slate-400"></span>
        <span class="ml-2">{{ counts.notinvited }}/{{ totals.notinvited }} Not Invited</span>
      </button>

      <button class="status-pill bg-violet-600 text-white hover:bg-violet-700"
        :class="statusFilter === 'all' ? 'ring-2 ring-violet-300' : ''" @click="setStatus('all')">
    ...
    <DataTable :rows="pagedRows" :columns="columns" :loading="loading" v-model:page="page" v-model:pageSize="pageSize"
      ...
      <template #cell-checkinBox="{ row }">
        <input type="checkbox" :checked="!!row.empCheckinStatus" class="h-4 w-4 cursor-default accent-emerald-600"
          disabled aria-label="checked-in" />
      </template>

      <!-- ✅ Column: Full Name (display + secondary line optional) -->
      <template #cell-fullName="{ row }">
        <div class="flex flex-col">
          <span class="font-medium text-slate-800">{{ row.empFullname }}</span>
    ...
    <!-- ===== Error Bar ===== -->
    <p v-if="error" class="mt-2 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700">
      {{ error }}
    </p>
  </div>
</template>
...
function mapInvite(ans) {
  ...
  if (a.includes('deny') || a.includes('reject') || a.includes('decline')) return 'declined'
...
const filteredBase = computed(() => {
  ...
  if (q) {
    list = list.filter((r) =>
      [String(r.empCompanyId), r.empFullname, r.empNickname].some((f) => f?.toLowerCase().includes(q)),
    )
  }

  if (statusFilter.value !== 'all') {
    list = list.filter((r) => mapInvite(r.empInviteStatus) === statusFilter.value)
  }

  if (selectedCompanyIds.value?.length) {
    const needles = selectedCompanyIds.value.map((x) => String(x).trim()).filter(Boolean)
    list = list.filter((r) => {
...
const totals = computed(() => {
  const t = { accepted: 0, declined: 0, pending: 0, notinvited: 0 }
  rows.value.forEach((r) => t[mapInvite(r.empInviteStatus)]++)
  return t
})

const counts = computed(() => {
  const c = { accepted: 0, declined: 0, pending: 0, notinvited: 0 }
...
/* ===== Table Columns ===== */
const columns = [
  { key: 'empCompanyId', label: 'ID', sortable: true, class: 'min-w-[120px] text-left' },
  { key: 'fullName', label: 'Name', sortable: true, class: 'min-w-[220px] text-left' },
  { key: 'empNickname', label: 'Nickname', sortable: true, class: 'min-w-[120px] text-left' },
  { key: 'empDepartment', label: 'Department', sortable: true, class: 'min-w-[180px] text-left' },
  { key: 'empTeam', label: 'Team', sortable: true, class: 'min-w-[160px] text-left' },
  { key: 'empPosition', label: 'Position', sortable: true, class: 'min-w-[200px] text-left' },
  { key: 'status', label: 'Invite', sortable: false, class: 'min-w-[120px] text-center' },
]
...
const sortedRows = computed(() => {
  ...
  const getter = (r) => (sortKey.value === 'fullName' ? (r.empFullname || '').trim() : r[sortKey.value])
