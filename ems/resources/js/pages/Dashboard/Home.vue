...
// ✅ ใช้ StatusCard ตัวเดียวแทน 3 ตัวเดิม
...
export default {
    components: {
        MagnifyingGlassIcon,
        StatusCard, // ✅ Register StatusCard
        DonutActualAttendance,
        GraphEventParticipation,
        Button,
        SearchBar,
    ...
    computed: {
        normalized() {
            return this.event.map((e) => {
                ...
                const catId =
                    e.evn_cat_id ?? e.evn_category_id ?? e.evn_category ?? "";
        ...
        filtered() {
            ...
            if (this.selectedDate?.start || this.selectedDate?.end) {
                arr = arr.filter((e) => {
                    ...
                    const startTime = this.selectedDate.start
                        ? new Date(this.selectedDate.start).getTime()
                        : null;
                    const endTime = this.selectedDate.end
                        ? new Date(this.selectedDate.end).getTime()
                        : null;
        ...
        empPaged() {
            ...
            switch (this.empSort.value) {
                case "name_az":
                    arr.sort((a, b) =>
                        a.emp_firstname.localeCompare(b.emp_firstname)
                    );
                    break;
                case "name_za":
                    arr.sort((a, b) =>
                        b.emp_firstname.localeCompare(a.emp_firstname)
                    );
                    break;
                case "department_az":
                    arr.sort((a, b) =>
        ...
        selectedEventData() {
            ...
            return this.normalized.find(
                (event) => (event.id || event.evn_id) == firstEventId
            );
        },
    ...
    methods: {
        // ✅ Handle StatusCard click events
        handleCardClick(type) {
            this.showEmployeesByStatus(type);
        },
        ...
        getAttendingProgress() {
            ...
            return Math.round(
                (this.chartData.attending / this.chartData.totalParticipation) *
                    251
            );
        },
        ...
        getNotAttendingProgress() {
            ...
            return Math.round(
                (this.chartData.notAttending /
                    this.chartData.totalParticipation) *
                    251
            );
        },
        ...
        getPendingProgress() {
            if (this.chartData.totalParticipation === 0) return 0;
            return Math.round(
                (this.chartData.pending / this.chartData.totalParticipation) *
                    251
            );
        },
        ...
        async fetchEventStatistics() {
            console.log(
                "🔄 fetchEventStatistics called with:",
            ...
            try {
                ...
                console.log(
                    "Sending POST /event-statistics with event_ids:",
                    eventIds
                );

                // ดึงสถิติจาก API
                const res = await axios.post("/event-statistics", {
                    event_ids: eventIds,
                });
                ...
                if (res.data) {
                    ...
                    console.log(
                        "Participation data updated:",
                        this.participationData
                    );
                    console.log(
                        "Participants count:",
                        this.eventParticipants.length
                    );
                }
            } catch (err) {
        ...
        toggleEventSelection(event) {
            ...
            this.selectAll =
                this.selectedEventIds.size === this.sorted.length &&
                this.sorted.length > 0;
        ...
        selectAllEvents(event) {
            ...
            if (this.selectAll) {
                ...
                this.selectedEventIds = new Set(
                    this.sorted.map((e) => e.id || e.evn_id)
                );
            } else {
        ...
        async showEmployeesByStatus(status) {
            ...
            try {
                ...
                if (status === "accepted") {
                    ...
                    filteredParticipants = this.eventParticipants.filter(
                        (participant) => {
                            return (
                                participant.status === "accepted" ||
                                participant.con_checkin_status === 1
                            );
                        }
                    );
                ...
                deduplicatedParticipants.sort((a, b) => {
                    ...
                    const empCompare = (a.emp_id || "").localeCompare(
                        b.emp_id || ""
                    );
                ...
                this.filteredEmployeesForTable = deduplicatedParticipants.map(
                    (participant) => ({
                        id: participant.id,
                ...
                console.log(
                    `Filtered ${filteredParticipants.length} raw participants`
                );
                console.log(
                    `After deduplication: ${this.filteredEmployeesForTable.length} unique participations for status: ${status}`
                );
                ...
                if (this.filteredEmployeesForTable.length > 0) {
                    console.log(
                        "Sample data (first 3):",
                        this.filteredEmployeesForTable.slice(0, 3)
                    );
                }
            } catch (error) {
