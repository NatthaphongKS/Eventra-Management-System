<!-- /**
 * ชื่อไฟล์: DepartmentCreate.vue
 * คำอธิบาย: Pop up สำหรับสร้างแผนก
 * Input: Department
 * Output: Create department
 * ชื่อผู้เขียน/แก้ไข: Natthaphong Kongsinl
 * วันที่จัดทำ/แก้ไข: 2026-03-10
 */ -->
<!-- resources/js/components/Department/DepartmentCreate.vue -->
<template>
  <div v-if="open" class="fixed inset-0 z-[70] flex items-center justify-center">
    ...
    <div class="relative w-[762px] h-auto p-12 rounded-[20px] bg-white shadow-xl">
      <div class="text-3xl font-semibold text-neutral-800">Create Department</div>
...
const emit = defineEmits<{
  ...
  (e: "submit", payload: { name: string; status: string }): void;
}>();
...
const showDup = computed(() => {
  if (!props.duplicate || !trimmed.value) return false;
  return props.duplicate(trimmed.value);
});
...
function submit() {
  ...
  emit("submit", { name: trimmed.value, status: status.value });
}
</script>
