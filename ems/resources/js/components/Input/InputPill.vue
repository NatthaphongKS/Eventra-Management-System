    ...
    <input v-bind="$attrs" :type="type" :inputmode="inputmode" :placeholder="placeholder" :disabled="disabled" class="rounded-[20px] border px-4 py-2.5 text-sm focus:outline-none focus:ring-2 transition
             placeholder-[#E1B4B4]" :class="[
              ...
              error
                ...
                : 'border-neutral-200 focus:ring-rose-300 focus:border-rose-400'
            ]" :value="modelValue" @input="$emit('update:modelValue', $event.target && $event.target.value)" />
    <p v-if="error" class="mt-1 text-xs text-red-500">{{ error }}</p>
  </div>
</template>

<script setup>
defineOptions({ inheritAttrs: false })  // สำคัญ!
defineProps({
