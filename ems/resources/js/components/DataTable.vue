 ...
 * คำอธิบาย: Component สำหรับแสดงตารางข้อมูล รองรับการแสดงผลข้อมูลแบบตาราง,
 * การแบ่งหน้า (Pagination), การเลือกแถวด้วย Checkbox และการจัดรูปแบบข้อมูล
 * รวมถึงสามารถกำหนดคอลัมน์และปรับแต่งการแสดงผลผ่าน slot ได้
 * Input: รับค่าจากหน้าแม่ผ่าน defineProps เช่น rows, columns, loading,
 * page, pageSize, totalItems, selectable และ modelValue
 * Output: ส่งอีเวนต์กลับไปยังหน้าแม่ผ่าน emit เช่น update:page, update:pageSize,
...
<template>
    <div class="relative">
        ...
        <div class="overflow-x-auto rounded-2xl border border-neutral-200" :class="{ 'opacity-50': loading }">
            <table class="w-full table-auto min-w-[800px]">
                <thead>
                    <tr class="bg-neutral-100 text-neutral-800">
                        ...
                        <th v-if="showRowNumber" class="w-12 py-3 text-center font-semibold">#</th>

                        <th v-for="col in columns" :key="col.key" class="py-3 px-3 font-semibold"
                        ...
                        <tr v-for="(row, index) in rows" :key="row[rowKey] ?? index"
                            ...
                            <td v-for="col in columns" :key="col.key" class="px-3 py-2 text-sm text-slate-800"
                                ...
                                <slot :name="`cell-${col.key}`" :value="getValue(row, col.key)" :row="row">
                                    <span :title="getValue(row, col.key)">
                                        {{ formatValue(row, col) }}
                                    </span>
                                </slot>
                            </td>
            ...
            <div v-if="totalItems > 0" class="mt-4 flex flex-col items-center gap-4 md:flex-row md:items-center">
                <div class="flex items-center gap-2 text-sm text-slate-700">
                    <slot name="footer-info" :from="fromItem" :to="toItem" :total="totalItems">
                        ...
                        <span> {{ fromItem }}-{{ toItem }} จาก {{ totalItems }} รายการ </span>
                    </slot>
                </div>
            </div>
            <div class="flex items-center justify-center gap-3 md:mx-auto">
                <button class="  text-red-700 transition hover:text-red-300 disabled:opacity-40" :disabled="page === 1"
                    @click="goToPage(page - 1)">
                    <icon icon="fluent:triangle-left-32-filled" width="25" height="25"></icon>
                </button>

                <template v-for="(it, idx) in pageItems">
                    <button v-if="it.type === 'page'" :key="`page-${it.value}`" class="pg-num !w-[41px] !h-[45px]"
                        :class="{ 'pg-active': it.value === page }" @click="goToPage(it.value)">
                        {{ it.value }}
                    </button>
                    <span v-else :key="`dots-${idx}`" class="pg-ellipsis !h-[45px] flex items-center justify-center">
                        <i class="dot"></i><i class="dot"></i><i class="dot"></i>
                    </span>
                </template>

                <button class=" text-red-700 transition hover:text-red-300 disabled:opacity-40"
                    :disabled="page === totalPages || totalPages === 0" @click="goToPage(page + 1)">
                    <icon icon="fluent:triangle-right-32-filled" width="25" height="25"></icon>
                </button>
            </div>
        </div>
    </div>
...
const fromItem = computed(() => (props.totalItems === 0 ? 0 : rowStartIndex.value + 1));
...
const isIndeterminate = computed(() => {
    const pickedCount = pageRowKeys.value.filter((key) => selectedSet.value.has(key))
        .length;
