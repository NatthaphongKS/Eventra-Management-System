<!-- /**
 * ชื่อไฟล์: PositionEdit.vue
 * คำอธิบาย: Pop up สำหรับแก้ไขตำแหน่ง
 * Input: Position Department Team
 * Output: Update position
 * ชื่อผู้เขียน/แก้ไข: Natthaphong Kongsinl
 * วันที่จัดทำ/แก้ไข: 2026-03-10
 */ -->
<!-- resources/js/components/Position/PositionEdit.vue -->
<template>
  <div v-if="open" class="fixed inset-0 z-[70] flex items-center justify-center">
    ...
    <div class="relative w-[762px] h-auto p-12 rounded-[20px] bg-white shadow-xl">
      <div class="text-3xl font-semibold text-neutral-800">Edit Position</div>
...
const props = defineProps<{
  ...
  position: {
    ...
    pst_name: string;
    pst_team_id: number;
    pst_delete_status: string;
  } | null;
  isDuplicate?: (name: string, teamId: number, currentId: number) => boolean;
...
const emit = defineEmits<{
  ...
  (e: "submit", payload: { id: number; name: string; teamId: number; status: string }): void;
}>();
...
watch(() => props.open, (v) => {
  if (v && props.position) {
    positionId.value = props.position.id;
    name.value = props.position.pst_name || "";
    status.value = props.position.pst_delete_status || "active";
    teamId.value = String(props.position.pst_team_id || "");

    // Find department from team
    const team = props.teams?.find(t => t.id === props.position.pst_team_id);
    departmentId.value = String(team?.tm_department_id || "");
...
const showDup = computed(() => {
  if (!props.isDuplicate || !trimmed.value || !teamId.value || !positionId.value) return false;
  return props.isDuplicate(trimmed.value, Number(teamId.value), positionId.value);
});
