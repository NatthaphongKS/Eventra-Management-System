        ...
        <table class="w-full table-auto">
            <thead>
                <tr class="bg-neutral-200">
                    ...
                    <th class="w-[13%] py-3 text-center">Deleted Date</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(cat, catIndex) in pagedRows" :key="cat.categoryName ?? catIndex"
                    class="odd:bg-white hover:bg-slate-100">
                    <td class="w-[6%] py-2 text-center text-sm text-slate-700 border-t">
                        {{ rowStartIndex + catIndex + 1 }}
                    </td>
                    <td class="w-[40%] py-2 text-left border-t">
                        <span class="block truncate text-sm text-slate-800">
                            {{ cat.categoryName || 'N/A' }}
                        </span>
                    </td>
                    <td class="w-[14%] py-2 text-center border-t">
                        <span class="block truncate text-sm text-slate-800">
                            {{ cat.categoryCreatedPerson || 'N/A' }}
                        </span>
                    </td>
                    <td class="w-[14%] py-2 text-center border-t">
                        <span class="block truncate text-sm text-slate-800">
                            {{ formatDate(cat.categoryCreatedDate) || 'N/A' }}
                        </span>
                    </td>
                    <td class="w-[13%] py-2 text-center border-t">
                        <span class="block truncate text-sm text-slate-800">
                            {{ cat.categoryDeletedPerson || 'N/A' }}
                        </span>
                    </td>
                    <td class="w-[13%] py-2 text-center border-t">
                        <span class="block truncate text-sm text-slate-800">
                            {{ formatDate(cat.categoryDeletedDate) || 'N/A' }}
                        </span>
                    </td>
                </tr>
...
<script>
export default {
    name: 'DataTableHistoryCategory',
    props: {
        /** ส่งเข้ามาเฉพาะ rows ที่ filter/sort แล้ว */
        rows: { type: Array, default: () => [
            {
                categoryName: 'Conference',
                categoryCreatedPerson: 'มอส',
                categoryCreatedDate: '2025-10-15 14:30:00',
                categoryDeletedPerson: 'โอม',
                categoryDeletedDate: '2025-10-16 09:00:00'
            },
            {
                categoryName: 'Workshop',
                categoryCreatedPerson: 'สมชาย',
                categoryCreatedDate: '2025-10-17 10:00:00',
                categoryDeletedPerson: 'สมหญิง',
                categoryDeletedDate: '2025-10-18 11:00:00'
            },
    ...
    methods: {
        onChangePageSize(e) {
            ...
            this.goToPage(1)
            this.$emit('updatePageSize:pageSize', next)
        },
        goToPage(p) {
            const t = this.totalPages
            let next = Number(p)
            ...
            if (next > t) next = t
            this.innerPage = next
            this.$emit('updatePage:page', next)
        },
