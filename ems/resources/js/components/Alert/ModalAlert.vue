                ...
                <transition name="pop">
                    <div class="relative max-w-[90vw] text-center shadow-2xl" :class="wrapperClass">
                       ...
                       hover:brightness-95 active:brightness-90 focus:outline-none focus:ring-4 focus:ring-red-200">
                            ...
                            <button ref="okBtn" type="button" @click="handleConfirm" :class="okBtnClass" autofocus>
                                {{ okText }}
                            </button>
                        </div>

                        <!-- ปิดโดยการกดนอกกรอบ (optional) -->
                        <button class="absolute inset-0 -z-10 cursor-default" aria-hidden="true"
                            @click.self="handleCancel"></button>
                    </div>
                </transition>
            </div>
        </transition>
