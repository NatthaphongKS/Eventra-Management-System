<template>
  <div
    class="w-full h-[516px] rounded-[20px] flex-col bg-white p-8 shadow-[0_10px_25px_-12px_rgba(0,0,0,0.25)]"
  >
    ...
    <div class="flex items-start">
      ...
      >
        <div class="relative z-10 flex min-w-max items-start justify-start gap-8">
          <div
          ...
          <div
            ...
            :key="index"
            class="relative flex min-w-[80px] flex-col items-center"
          >
            <div class="flex h-[300px] items-end gap-2 mb-3">
              <div
                class="group relative w-10 cursor-pointer rounded-xl transition-all duration-500 hover:-translate-y-0.5 flex flex-col justify-end items-center pb-2"
                :style="{ height: getBarHeight(item.attending) + '%' }"
                style="background: linear-gradient(180deg, #dcfce7, #00a73d)"
              >
                <span
                  v-if="item.attending > 0"
                  class="text-[11px] font-bold text-white bg-black/20 px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                >
                  {{ item.attending }}
                </span>
              </div>
            ...
            <div
              class="max-w-[90px] break-words text-center text-[15px] font-medium text-neutral-700"
            >
              {{ item.name }}
            </div>
...
export default {
  ...
  methods: {
    ...
    getBarHeight(value) {
      ...
      // เพิ่ม logic: ถ้ามีค่าแต่ขีดต่ำมาก ให้ขั้นต่ำ 10% เพื่อให้พอเห็นตัวเลข (optional)
      const pct = (value / maxValue) * 100;
      return value > 0 && pct < 12 ? 12 : pct;
    },
    onViewTypeChange() {
      this.updateDisplayData();
    },
